...
function SectionHeader({ title }) {
  return <div className="flex items-center gap-2 mt-4 mb-2 text-sm font-semibold text-gray-700"><span className="w-1.5 h-4 bg-gray-300 rounded"/>{title}</div>;
}

export function ModuleBlock({ module, store, setStore }) {
  const [open, setOpen] = useState(true);
  const stats = useMemo(() => {
    let time = 0, pts = 0, total = 0, done = 0;
    module.sections.forEach((sec) => sec.items.forEach((it) => {
      total++; if (store.items[it.id]?.done) done++;
      if (it.meta?.timeMin) time += it.meta.timeMin;
      if (it.meta?.points) pts += it.meta.points;
    }));
    return { time, pts, total, done, pct: total ? Math.round((done / total) * 100) : 0 };
  }, [module, store]);
      ...
      <button onClick={() => setOpen((o) => !o)} className="w-full text-left px-4 py-3 flex items-center justify-between">
        <div>
          ...
          <div className="text-xs text-gray-500 mt-0.5 flex gap-2 items-center">
            ...
            <Pill><Icon name="points" className="w-3.5 h-3.5" /> {stats.pts} pts</Pill>
        ...
        <div className="px-4 pb-4">
          {module.sections.map((sec) => (
            <div key={sec.id}>
              ...
              {sec.items.map((it) => (
                <ItemRow key={it.id} item={it} state={store.items[it.id]} onToggleDone={toggleDone} onSaveNote={saveNote} />
              ))}
            </div>
          ))}
        </div>
